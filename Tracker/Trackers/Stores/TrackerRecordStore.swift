...
final class TrackerRecordStore {
	...
	func fetchAll() throws -> [TrackerRecord] {
		...
		return records.compactMap { object in
			guard
	...
	private func fetchRecordObject(trackerID: UUID, date: Date, in context: NSManagedObjectContext) throws -> NSManagedObject? {
		let request = NSFetchRequest<NSManagedObject>(entityName: RecordEntity.name)
		request.fetchLimit = 1
		request.predicate = NSPredicate(format: "%K.%K == %@ AND %K == %@", RecordEntity.tracker, TrackerEntity.id, trackerID as CVarArg, RecordEntity.date, date as CVarArg)
		return try context.fetch(request).first
	}
}
